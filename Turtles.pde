...
class Robot extends Turtle {
  ...
  void sendMessage(Robot bob, int type, float[] args) {
    ...
    if (bob != null) {
      // create the message...
      Message msg = new Message(type, who, args);
  ...
  void askForEnergy(Robot bob, float qty) {
    // check that bob is a base
    if (bob.breed == BASE) {
      // build the message...
      float[] args = new float[1];
      args[0] = qty;
      Message msg = new Message(ASK_FOR_ENERGY, who, args);
      // ...and add it to bob's messages queue
      bob.messages.add(msg);
    }
  }
  ...
  void askForBullets(Robot bob, int qty) {
    ...
    if (bob.breed == BASE) {
      ...
      float[] args = new float[1];
      args[0] = qty;
      Message msg = new Message(ASK_FOR_BULLETS, who, args);
      // ...and add it to bob's messages queue
      bob. messages.add(msg);
    }
  }
  ...
  void informAboutFood(Robot bob, PVector p) {
    ...
    if (bob != null) {
      ...
      args[0] = p.x;
      args[1] = p.y;
      Message msg = new Message(INFORM_ABOUT_FOOD, who, args);
      // ...and add it to bob's messages queue
      bob. messages.add(msg);
    }
  }
  ...
  void informAboutXYTarget(Robot bob, PVector p) {
    ...
    if (bob != null) {
      ...
      args[0] = p.x;
      args[1] = p.y;
      Message msg = new Message(INFORM_ABOUT_XYTARGET, who, args);
      // ...and add it to bob's messages queue
      bob. messages.add(msg);
    }
  }
  ...
  void informAboutTarget(Robot bob, Robot target) {
    // check that bob and target both exist
    if ((bob != null) && (target != null)) {
      // build the message...
...
class Base extends Robot {
  ...
  void giveEnergy(int agt, float nrj) {
    ...
    if (bob != null) {
      ...
      if ((game.distance(this, bob) <= 2) && (nrj <= energy)) {
  ...
  void giveBullets(int agt, float qty) {
    ...
    if ((bob != null) && (bob.breed == LAUNCHER)) {
      ...
      if ((game.distance(this, bob) <= 2)
        && (bob.bullets + qty <= launcherMaxBullets)
